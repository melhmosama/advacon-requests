...
window.warehouseReviewImage=async(id,button)=>{await AdvaconUI.busy(button,async()=>{try{const value=rows?.find(r=>r.id===id)?.args?.p_image_url;if(!value)return;const url=await whResolveImage(value);openLightbox(url)}catch(e){toast(e.message,'error')}})};
